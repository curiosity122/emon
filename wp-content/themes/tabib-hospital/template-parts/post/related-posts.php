<?php 
if ( ! function_exists( 'tabib_hospital_related_posts_function' ) ) {
	function tabib_hospital_related_posts_function() {
		wp_reset_postdata();
		global $post;
		...
		$args = array(
			...
			'posts_per_page'    => absint( get_theme_mod( 'tabib_hospital_change_related_posts_number', '3' ) ),
		);
		// Related by categories
		if ( get_theme_mod( 'tabib_hospital_show_related_post', 'categories' ) == 'categories' ) {
		...
		// Related by tags
		if ( get_theme_mod( 'tabib_hospital_show_related_post', 'categories' ) == 'tags' ) {
...
$related_posts = tabib_hospital_related_posts_function(); ?>

<?php if ( $related_posts->have_posts() ): ?>

	<div class="related-posts clearfix py-3">
		<?php if ( get_theme_mod('tabib_hospital_change_related_post_title','Related Posts') != '' ) {?>
			<h2 class="related-posts-main-title"><?php echo esc_html( get_theme_mod('tabib_hospital_change_related_post_title',__('Related Posts','tabib-hospital')) ); ?></h2>
		<?php }?>
		<div class="row">
